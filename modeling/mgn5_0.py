...
from modeling.view_net import view_net,view_mat

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

view_model = view_net().to(device)
# 导入模型参数
model_params_path = "/data/wz/weights/view_res50.pkl"
net_params = torch.load(model_params_path)
# 把加载的参数应用到模型中
view_model.load_state_dict(net_params, strict=False)
...
class MGN(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.p0 = nn.Sequential(view_model.p1[0], view_model.p1[1])
        # mgn part-1 global
        ...
        # fc softmax loss
        self.fc_view_2048_8 = nn.Linear(2048,8)
        ...
        #################################BN
        self.bottleneck256 = nn.BatchNorm1d(256)
        self.bottleneck2048= nn.BatchNorm1d(2048)
        self.bottleneck256.bias.requires_grad_(False)
        self.bottleneck2048.bias.requires_grad_(False)# no shift
        self.weights_init_kaiming(self.bottleneck256)
        self.weights_init_kaiming(self.bottleneck2048)
        #################################
        self.dropout = torch.nn.Dropout(p=0.5)



        self.view_dist = torch.Tensor([
            [1., 0.8, 0.2, 0.1, 0., 0.1, 0.2, 0.8],
            ...
            [0.8, 0.8, 0.2, 0.1, 0.1, 0.1, 0.3, 1.]
        ]
        )
        # self.view_dist = torch.eye(8)
        ################################
    ...
    def forward(self, x):
        ...
        zg_p0 = nn.functional.adaptive_avg_pool2d(p0, (1, 1))
        # zg_p1 = nn.functional.adaptive_avg_pool2d(p1, (1, 1))
        ...
        fg_p0_l = self.bottleneck2048(zg_p0.squeeze(dim=3).squeeze(dim=2))
        l_p1 = self.fc_id_2048_0(self.bottleneck2048(zg_p1.squeeze(dim=3).squeeze(dim=2)))  # L_softmax^G
        l_p2 = self.fc_id_2048_1(self.bottleneck2048(zg_p2.squeeze(dim=3).squeeze(dim=2)))  # L_softmax^P2
        l_p3 = self.fc_id_2048_2(self.bottleneck2048(zg_p3.squeeze(dim=3).squeeze(dim=2)))  # L_softmax^P3
        l0_p2 = self.fc_id_256_1_0(self.bottleneck256(f0_p2))  # L_softmax0^P2
        l1_p2 = self.fc_id_256_1_1(self.bottleneck256(f1_p2))  # L_softmax1^P2
        l0_p3 = self.fc_id_256_2_0(self.bottleneck256(f0_p3))  # L_softmax0^P3
        l1_p3 = self.fc_id_256_2_1(self.bottleneck256(f1_p3))  # L_softmax1^P3
        l2_p3 = self.fc_id_256_2_2(self.bottleneck256(f2_p3))  # L_softmax2^P3


        l_p1 = self.dropout(l_p1)
        l_p2 = self.dropout(l_p2)
        l_p3 = self.dropout(l_p3)
        l0_p2 = self.dropout(l0_p2)
        l1_p2 = self.dropout(l1_p2)
        l0_p3 = self.dropout(l0_p3)
        l1_p3 = self.dropout(l1_p3)
        l2_p3 = self.dropout(l2_p3)

        l_p0 = self.fc_view_2048_8(fg_p0_l)
        _, view_max = torch.max(l_p0.data, 1)
        view_dist_out = view_mat(view_max, self.view_dist)


        predict = torch.cat([fg_p1, fg_p2, fg_p3, f0_p2, f1_p2, f0_p3, f1_p3, f2_p3], dim=1)

        return predict, fg_p1, fg_p2, fg_p3, \
               l_p1, l_p2, l_p3, \
               l0_p2, l1_p2, l0_p3, l1_p3, l2_p3,\
               view_dist_out,l_p0
