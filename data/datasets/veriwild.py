...
from .bases import BaseImageDataset1

class VeRiWild(BaseImageDataset1):
    dataset_dir = 'VeRiWild'

    folders = {'small': 3000, 'medium': 5000, 'large': 10000}

    def __init__(self, root='/data/wangz/', verbose=True, add_mask=False, num_instance=4, folder='small', **kwargs):
        ...
        gallery = self._process_txt(
            osp.join(self.dataset_dir, 'train_test_split', 'test_{}_id.txt'.format(self.folders[self.folder])),
            self.gallery_dir, relabel=False)
        query = self._process_txt(
            osp.join(self.dataset_dir, 'train_test_split', 'test_{}_id_query.txt'.format(self.folders[self.folder])),
        ...
        self.num_train_pids, self.num_train_imgs, self.num_train_cams = self.get_imagedata_info1(self.train)
        self.num_query_pids, self.num_query_imgs, self.num_query_cams = self.get_imagedata_info1(self.query)
        self.num_gallery_pids, self.num_gallery_imgs, self.num_gallery_cams = self.get_imagedata_info1(self.gallery)
